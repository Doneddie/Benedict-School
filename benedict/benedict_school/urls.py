...
<<<<<<< HEAD
    ...
    path(
        "parent/<int:pk>/", ParentDetailView.as_view(), name="parent-detail"
    ),  # Parent detail page
    path(
        "parent/new/", ParentCreateView.as_view(), name="parent-create"
    ),  # Create a new parent
    path(
        "parent/<int:pk>/edit/", ParentUpdateView.as_view(), name="parent-update"
    ),  # Edit a parent
    path(
...
>>>>>>> a1b3e24 (Handled a few forms)
    ...
    path(
        "parent/<int:parent_id>/child/new/",
        ChildCreateView.as_view(),
        name="child-create",
    ),  # Create a new child
    path(
        "child/<int:pk>/", ChildDetailView.as_view(), name="child-detail"
    ),  # Child detail page
    path(
        "child/<int:pk>/edit/", ChildUpdateView.as_view(), name="child-update"
    ),  # Edit a child
    path(
        "child/<int:pk>/delete/", ChildDeleteView.as_view(), name="child-delete"
    ),  # Delete a child
    # Pupil Application URLs
    path(
        "child/<int:child_id>/application/new/",
        PupilApplicationCreateView.as_view(),
        name="pupil-application-create",
    ),  # Create a pupil application
    path(
        "application/<int:pk>/",
        PupilApplicationDetailView.as_view(),
        name="pupil-application-detail",
    ),  # View a specific pupil application
    # ---------- Event URLs ----------
    path("events/", EventListView.as_view(), name="event-list"),  # List all events
    path(
        "event/<int:pk>/", EventDetailView.as_view(), name="event-detail"
    ),  # Event detail page
    # ---------- Activity URLs ----------
    path(
    ...
    path(
        "activity/<int:pk>/", ActivityDetailView.as_view(), name="activity-detail"
